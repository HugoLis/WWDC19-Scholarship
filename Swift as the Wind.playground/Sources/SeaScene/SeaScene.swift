...
public class SeaScene: SKScene, SKPhysicsContactDelegate {
    
    //wind vector
    var wind = CGVector(dx: 0, dy: 500)
    var boat: SKSpriteNode?
    var sail: SKSpriteNode?
    var shipWheel: SKSpriteNode?
    var sailWheel: SKSpriteNode?
    var waterTest: SKSpriteNode?
    var lakeIn: SKSpriteNode?
    var lakeOut: SKSpriteNode?
    var pauseCurtain: SKSpriteNode?
    var coin1: SKSpriteNode?
    var coin2: SKSpriteNode?
    var coin3: SKSpriteNode?
    var boost: SKSpriteNode?
    var boost2: SKSpriteNode?
    var finishLine: SKSpriteNode?
    var finishLineSon: SKSpriteNode?
    
    var coinHolder1: SKSpriteNode?
    var coinHolder2: SKSpriteNode?
    var coinHolder3: SKSpriteNode?
    var myCamera: SKCameraNode?
    var waterShader: SKShader?
    var sailShader: SKShader?
    let backgroundMusic = SKAudioNode(fileNamed: "Sounds/theme.m4a")
    let pausedMusic = SKAudioNode(fileNamed: "Sounds/paused.m4a")
    let fireworksSound = SKAudioNode(fileNamed: "Sounds/fireworksSound")
    var boatTrail = SKEmitterNode(fileNamed: "Particles/boatTrail")
    var boatBoostTrail = SKEmitterNode(fileNamed: "Particles/boatBoostTrail")
    var windParticle = SKEmitterNode(fileNamed: "Particles/windParticle")
    var swirlParticle = SKEmitterNode(fileNamed: "Particles/swirlParticle")
    var swirlParticle2 = SKEmitterNode(fileNamed: "Particles/swirlParticle")
    
    var pauseButton = PauseButton()
    var playAfterPauseButton = PlayAfterPauseButton()
    var restartButton = RestartButton()
    var soundOnButton = SoundOnButton()
    var soundOffButton = SoundOffButton()
    var homeButton = HomeButton()
    
    var boatWind: CGVector?
    var boatAngle: CGFloat?
    var boatIsMovingForward: Bool?
    var boatIsPushedForward: Bool?
    var apparentWind: CGVector?
    var apparentWindAngle: CGFloat?
    var trueSailAngle: CGFloat?
    var finalLift: CGVector?
    var finalPush: CGVector?
    var width: CGFloat?
    var height: CGFloat?
    var fingers = [String?](repeating: nil, count:5)
    var touchBeganPosition = [0, 0, 0, 0, 0] //0 = nothing, 1 = left, 2 = right
    
    let cameraBoundBoxSize: CGFloat = 50
    let wheelsYOffset: CGFloat = 5
    var wheelsXOffset: CGFloat = 150
    var wheelsOriginOffset: CGFloat = 0
    let dThetaLimit: CGFloat = 0.4
    var isSailWiggling = false
    var sailWheelVelocity: CGFloat = 72
    var shipWheelVelocity: CGFloat = 72
    var shipWheelTouchLocation: CGPoint?
    var previousShipWheelTouchLocation: CGPoint?
    var sailWheelTouchLocation: CGPoint?
    var previousSailWheelTouchLocation: CGPoint?
    var sailReachedLimit = false
    var lastSailReachedLimit = false
    var sailWheelAngleBeforeLimit: CGFloat = 0
    var fingerOnSailWheel = false
    var coinCounter: Int = 0
    var coinScale: CGFloat = 0.25
    var pauseButtonScale: CGFloat = 1.3
    var topItemsYOffset: CGFloat = 0.8
    var cameraScale: CGFloat = 0.85
    var cameraScaleAmplifier: CGFloat = 1.2
    var pauseScreenYDivider: CGFloat = 4
    var boostTrailActive = false
    var boostTrailDivider: CGFloat = 1
    
    var swirlGravityField = SKFieldNode.radialGravityField()
    var swirlVortexField = SKFieldNode.vortexField()
    var swirlGravityBoatField = SKFieldNode.radialGravityField()
    let swirlBoatGravity: Float = 0.2
    var swirlTorque: CGFloat = 10
    var swirlDirection: CGFloat = 1
    var driftIsEnabled = false
    var lastTimeRecord = Date().timeIntervalSinceReferenceDate
    var swirlTimer = Date().timeIntervalSinceReferenceDate
    var isPreStart = true
    var isGamePaused = false
    var shouldRunPreStartAnimation = false
    
    var shouldCalculateCongratsScreen = false
    var congratsRectangle: SKShapeNode?
    var congratsLabel: SKLabelNode?
    var playAgainLabel1: SKLabelNode?
    var playAgainLabel2: SKLabelNode?
    var finalHomeButton = FinalHomeButton()
    ...
    override public func didMove(to view: SKView) {
        ...
        sailWheel?.physicsBody?.angularDamping = 12 //20
        ...
        shipWheel?.position = CGPoint(x: -(width!/2) + wheelsXOffset , y: -(height!/4) + 2 * wheelsYOffset) //+5
        sailWheel?.position = CGPoint(x: (width!/2) - wheelsXOffset , y: -(height!/4) + 2 * wheelsYOffset) //+5
        coinScale = 0.25
        cameraScale = 0.85
        ...
        lakeIn?.texture = SKTexture(imageNamed: "Lakes/lakeIn3")
        lakeIn?.setScale(2/3)
        lakeOut = self.childNode(withName: "lakeOut") as? SKSpriteNode
        lakeOut?.texture = SKTexture(imageNamed: "Lakes/lakeOut3")
        ...
        boost2 = self.childNode(withName: "boost2") as? SKSpriteNode
        boost2?.texture = SKTexture(imageNamed: "Coins/boost")
        boost2?.physicsBody = SKPhysicsBody(circleOfRadius: (boost2?.size.width)!/2)
        boost2?.physicsBody?.isDynamic = false
        boost2?.physicsBody?.categoryBitMask = PhysicsCategory.boost
        boost2?.physicsBody?.contactTestBitMask = PhysicsCategory.boat

        //Swirl repositioned at the end of didMove
        swirlParticle?.position = CGPoint(x: -256, y: 100)
        swirlParticle?.zPosition = 80
        swirlParticle?.fieldBitMask = PhysicsCategory.particleField
        
        swirlParticle?.physicsBody = SKPhysicsBody(circleOfRadius: 0.7 * (swirlParticle?.particlePositionRange.dx)!)
        swirlParticle?.physicsBody?.collisionBitMask = PhysicsCategory.none
        swirlParticle?.physicsBody?.categoryBitMask = PhysicsCategory.swirl
        swirlParticle?.physicsBody?.contactTestBitMask = PhysicsCategory.boat
        swirlParticle?.physicsBody?.fieldBitMask = PhysicsCategory.none
        swirlParticle?.physicsBody?.isDynamic = false
        
        swirlParticle2?.fieldBitMask = PhysicsCategory.particleField
        swirlParticle2?.zRotation = CGFloat.pi/4
        swirlParticle?.addChild(swirlParticle2!)
    
        swirlGravityField.categoryBitMask = PhysicsCategory.particleField
        swirlGravityField.strength = 0.6
        swirlGravityField.minimumRadius = Float((swirlParticle?.particlePositionRange.dx)!/1.2)
        
        swirlVortexField.categoryBitMask = PhysicsCategory.particleField
        swirlVortexField.minimumRadius = Float((swirlParticle?.particlePositionRange.dx)!/1.2)
        swirlVortexField.strength = 2
        self.addChild(swirlParticle!)
        
        swirlParticle?.addChild(swirlVortexField)
        swirlParticle?.addChild(swirlGravityField)
        let rotateAction = SKAction.rotate(byAngle: 2 * CGFloat.pi, duration: 3)
        swirlParticle?.run(SKAction.repeatForever(rotateAction))
        
        swirlGravityBoatField.categoryBitMask = PhysicsCategory.boatField
        swirlGravityBoatField.minimumRadius = 30
        swirlGravityBoatField.strength = swirlBoatGravity
        swirlGravityBoatField.region = SKRegion(radius: Float(1.05 * (swirlParticle?.particlePositionRange.dx)!))
        swirlParticle?.addChild(swirlGravityBoatField)
        ...
        let lake3 = Paths(lakeNumber: 3)
        lakeIn?.physicsBody = SKPhysicsBody(edgeLoopFrom: lake3.shape.cgPath)
    ...
    override public func update(_ currentTime: TimeInterval) {
        ...
        if shouldCalculateCongratsScreen == true {
            congratsRectangle?.position = CGPoint(x: 0, y: height!/10)
                
            if wheelsXOffset < 120 { //narrow half
                congratsRectangle?.setScale(0.75)
                congratsLabel?.fontSize = 35
                playAgainLabel1?.fontSize = 23
                playAgainLabel2?.fontSize = 23
            }
            else { // landscape
            ...
            if  2.5 * (shipWheel?.size.width)! > height! { //mini mode
                congratsRectangle?.position = CGPoint(x: 0, y: 0)
                congratsLabel?.position = CGPoint(x: 0, y: (congratsRectangle?.frame.size.height)!/4.5)
                playAgainLabel1?.position = CGPoint(x: 0, y: (congratsRectangle?.frame.size.height)!/20)
                playAgainLabel2?.position = CGPoint(x: 0, y: -(congratsRectangle?.frame.size.height)!/15)
                finalHomeButton.position = CGPoint(x: 0, y: -(congratsRectangle?.frame.size.height)!/3.5)
            }
            else {
        ...
        if isPreStart == false && isGamePaused == false {
            ...
            //update camera position in relation to boat position
            ...
            //set max limit to be added to boat nose center to prevent boat from hitting shipWheel and going out of camera bounds
            ...
            if ((boat?.physicsBody?.velocity)?.length())! < CGFloat(0.1) {
                ...
                boatBoostTrail?.particleBirthRate = 0
                boatBoostTrail?.particleLifetime = 0
                boatBoostTrail?.particleScaleSpeed = 0
                boatBoostTrail?.particleScaleRange = 0
            }
            else {
                ...
                if boostTrailActive == true {
                    boatBoostTrail?.particleBirthRate = (boatTrail?.particleBirthRate)! * 0.7
                    boatBoostTrail?.particleLifetime = (boatTrail?.particleLifetime)! / boostTrailDivider
                    boatBoostTrail?.particleLifetimeRange = (boatTrail?.particleLifetimeRange)! / boostTrailDivider
                    boatBoostTrail?.particleScaleSpeed = (boatTrail?.particleScaleSpeed)! * 1.4
                    boatBoostTrail?.particleScaleRange = (boatTrail?.particleScaleRange)!
                    
                    boostTrailDivider = boostTrailDivider * 1.025
                }
                else {
                    boatBoostTrail?.particleBirthRate = 0
                    boostTrailDivider = 1
    ...
    public func boatDidCollideWithFinishLine(boat: SKSpriteNode, finishLineSon: SKSpriteNode){
        if (finishLineSon.parent != nil) {
            ...
            SKTexture.preload([SKTexture(imageNamed: "Menu/pressedLevel2Button")], withCompletionHandler: {
                //print("complete")
            } )
            
            self.run(SKAction.wait(forDuration: 1)){
                self.pausedMusic.run(SKAction.changeVolume(to: 0, duration: 1))
                self.backgroundMusic.run(SKAction.changeVolume(to: 0, duration: 1))
            }
            ...
            self.run(SKAction.wait(forDuration: 2)){
                ...
                if let menuScene = MenuScene(fileNamed: "menuScene"){
                    ...
                    menuScene.level1LastScore = self.coinCounter
                    if level2IsUnlocked == false{
                        level2WasJustUnlocked = true
                    }
